  ...
  return (
    <div className={`rounded-xl p-3 transition-all ${
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {player.hasFinished && <span className="seal text-[8px] w-4 h-4 rounded-[3px] leading-none ml-auto"><Ruby>着</Ruby></span>}
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-washi/75">
        ...
        <div className="flex items-center gap-1"><Icon name="fish" size={13} className="text-ai-200" /> {player.caughtFish.length}<Ruby>匹</Ruby></div>
        <div className="truncate">📍 <Ruby>{node?.name || '?'}</Ruby></div>
      ...
      {isActive && (
        <div className="mt-2 pt-2 border-t border-kin-500/15 text-xs text-washi/55 space-y-0.5">
          <div className="flex items-center gap-1.5"><Icon name="rod" size={13} className="text-washi/70" /> <Ruby>{getEquipmentName('rod', getEquippedLevel(player.equipment, 'rod')) || '未装着'}</Ruby></div>
          <div className="flex items-center gap-1.5"><Icon name="reel" size={13} className="text-washi/70" /> <Ruby>{getEquipmentName('reel', getEquippedLevel(player.equipment, 'reel')) || '未装着'}</Ruby></div>
          <div className="flex items-center gap-1.5"><Icon name="lure" size={13} className="text-washi/70" /> <Ruby>{getEquipmentName('lure', getEquippedLevel(player.equipment, 'lure')) || '未装着'}</Ruby></div>
        </div>
      )}
      ...
      {player.fishBonusTurnsLeft > 0 && (
        <div className="mt-1 text-xs text-kin-300 flex items-center gap-1">
          ✦ <Ruby>釣り</Ruby>{player.fishBonusMultiplier}<Ruby>倍</Ruby> (<Ruby>残</Ruby>{player.fishBonusTurnsLeft}T)
        </div>
      )}
    </div>
  );
