...
export const LAND_PATHS: string[] = [
  landPath(inRegions(['hokkaido']), 9),
  landPath(inRegions(['tohoku', 'kanto', 'chubu', 'kinki', 'chugoku']), 8),
  landPath(inRegions(['shikoku']), 8),
  landPath(BOARD_NODES.filter(n => n.region === 'kyushu' && n.y < 160), 8),
];

// 南西諸島（種子島〜那覇・ゴール）は小さな島として個別に描く
export const ISLANDS = BOARD_NODES
  .filter(n => n.region === 'kyushu' && n.y >= 160)
  .map(n => ({ cx: n.x, cy: n.y, rx: 7, ry: 6 }));
...
export const REGION_LABELS = Object.entries(REGION_NAMES).map(([region, name]) => {
  const ns = BOARD_NODES.filter(n => n.region === region && n.y < 160);
