    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="panel-ai relative rounded-2xl p-6 max-w-xs w-[85%]" onClick={e => e.stopPropagation()}>
        ...
        <div className="text-center">
          ...
          <h3 className="text-xl font-mincho font-bold text-kin-300"><Ruby>{caught ? fish.name : '???'}</Ruby></h3>
          <p className={`text-sm font-bold ${rarity.color}`}>{rarity.label}</p>
        </div>

        {caught ? (
          <div className="mt-4 space-y-2 text-sm">
            <p className="text-washi/70"><Ruby>{fish.description}</Ruby></p>

            <div className="flex flex-wrap justify-center gap-1.5 mt-3">
              <span className="text-xs bg-ai-500/20 text-ai-200 border border-ai-500/30 px-2 py-0.5 rounded-full"><Ruby>{fish.habitat}</Ruby></span>
              <span className="text-xs bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-full"><Ruby>{fish.season}</Ruby></span>
            ...
            {fish.wikiUrl && (
              ...
              >
                ...
                <Ruby>詳しく見る（Wikipedia）</Ruby>
              </a>
            )}
          </div>
        ) : (
          <p className="mt-4 text-sm text-washi/50 text-center"><Ruby>まだ釣っていません</Ruby></p>
        )}
